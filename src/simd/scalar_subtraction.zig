...
pub fn compute(input_vector: []const f32, subtrahend: f32, output_vector: []f32) void {
    ...
    inline while (vector_len >= constants.min_vector_len) : (vector_len /= 2) {
        if (rest_input_vector.len >= vector_len) {
            computeFixed(vector_len, rest_input_vector, subtrahend, rest_output_vector);
    ...
    if (rest_input_vector.len > 0) {
        computeFixed(1, rest_input_vector, subtrahend, rest_output_vector);
    }
}
...
fn computeFixed(
    ...
    const subtrahend_vector: @Vector(vector_len, f32) = @splat(subtrahend);

    var offset: usize = 0;

    while (offset + vector_len <= input_vector.len) : (offset += vector_len) {
        output_vector[offset..][0..vector_len].* =
            input_vector[offset..][0..vector_len].* - subtrahend_vector;
    }
}
...
test "scalar_subtraction" {
    ...
    for (0..2) |vector_len_subtrahend| {
        ...
        compute(input_vector, subtrahend, output_vector);

        for (0..output_vector.len) |index| {
            try std.testing.expectApproxEqRel(
                @as(f32, @floatFromInt(index + 1)) - subtrahend,
                output_vector[index],

...
pub fn compute(input_matrix: []const f32, input_vector: []const f32, output_vector: []f32) void {
    std.debug.assert(input_matrix.len == output_vector.len * input_vector.len);
    ...
    for (output_vector, 0..) |*output, index| {
        output.* = dot_product.compute(
            input_matrix[index * input_vector.len ..][0..input_vector.len],
            input_vector,
        );
    }
}
...
test "matrix_vector_product" {
    ...
    compute(&input_matrix, &input_vector, output_vector);
